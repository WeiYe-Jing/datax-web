...
```
由于启动时依赖`jps`命令，在某些环境中特别是openjdk的，可能没有这个命令，可以通过如下方式安装
* 查看openjdk版本，`rpm -qa | grep openjdk`，假如输入结果是
```
  java-1.8.0-openjdk-headless-1.8.0.282.b08-1.el7_9.x86_64
  java-1.8.0-openjdk-1.8.0.282.b08-1.el7_9.x86_64
```
* 安装对应版本的devel，比如上面的结果对应的是`yum install -y  java-1.8.0-openjdk-devel`
...
### 10) 原来的datax-web只支持mysql，新增oracle分支专为datax-admin是oracle处理的。建表语句见`bin/db/datax_web_oracle.sql`

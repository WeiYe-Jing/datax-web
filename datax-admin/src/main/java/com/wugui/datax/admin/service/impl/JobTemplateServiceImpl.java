...
public class JobTemplateServiceImpl implements JobTemplateService {
    ...
    public Map<String, Object> pageList(int start, int length, int jobGroup, String jobDesc, String executorHandler, String author) {
        ...
        List<JobUser> allUser = jobUserMapper.findAll("");

        Map<Integer, String> mapUsers = new HashMap<>();
        allUser.forEach(user -> {
            mapUsers.put(user.getId(), user.getNickname());
        });

        list.forEach(jobTemplate -> {
            String[] authors = jobTemplate.getAuthor().split(",");
    ...
    public ReturnT<String> update(JobTemplate jobTemplate) {
        ...
        exists_jobTemplate.setParentJobId(jobTemplate.getParentJobId());

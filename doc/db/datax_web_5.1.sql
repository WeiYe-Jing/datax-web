...
为什么有这个sql文件
1、数据库编码问题
MySQL在5.5.3之后增加了这个utf8的编码，mb4就是most bytes 4的意思，专门用来兼容四字节的unicode。
好在utf8是utf8的超集，除了将编码改为utf8外不需要做其他转换。
当然，为了节省空间，一般情况下使用utf8也就够了。
2、datetime 默认值
使用current_timestamp作为datetime的默认值，只有在5.6之后的版本才支持。
强烈建议升级到 >5.6
强烈建议升级到 >5.6
强烈建议升级到 >5.6
*/
...
CREATE TABLE `job_template`  (
  ...
  `inc_start_time` datetime NULL COMMENT '增量初始时间',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = Dynamic;
